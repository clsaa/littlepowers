...
If the specification has neither explicit approval nor a current
policy-matching Review Lease resolution followed by successful Contract
binding, present it and wait instead of starting the design.
...
`park-review --artifact-key design` with the inherited scope state and
open-question count. For a blocking policy, present the design for review and
stop: summarize architecture and key tradeoffs, name the artifact path, and
name `writing-plans` as the next phase. Resolve only an exact eligible automatic
gate before invoking `writing-plans`. A correction cancels the gate, revises
and checkpoints the same artifact, then parks it again instead of advancing.
